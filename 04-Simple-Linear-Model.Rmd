...
This is the same as a simple example with a rent (for an apartment): 
$$
...
$$
y_i = \beta_0 + \beta_1 x_i + u_i\quad \text{black line}
$$
...
invoices %>%
  mutate(
    y_hat_manual = 0.0001 - 5 * Invoices,
    residual_manual = Time - y_hat_manual,
    y_hat_OLS = 0.64171 + 0.01129 * Invoices,
    residual_OLS = Time - y_hat_OLS
  ) %>%
  summarise(
    RSS_manual = sum(residual_manual ^ 2),
    RSS_OLS = sum(residual_OLS ^ 2)
  )
```

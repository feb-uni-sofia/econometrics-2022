...
\text{Annahme } e_i \sim N(0, \sigma^2)
$$
...
sim_df <- sim_df %>%
  mutate(
    ...
    ## n(): Anzahl der Beobachtungen im Datensatz (oder der Gruppe)
...
```{r}
lm(kid_score ~ 1 + mom_hs, data = stichprobe1)
```
...
```{r}
lm(kid_score ~ 1 + mom_hs, data = stichprobe2)
```
...
The data set `sim_coeff` now contains estimated coefficients ($\hat{\beta_0}$ and $\hat{\beta}$) for every sample. To plot the distribution of $\hat{\beta}}$ we filter the data set so that we keep only the raw where `term == "mom_hs"` (i.e our estimates for $\beta$).
...
steigungskoeff %>%
  ...
  geom_boxplot(alpha = 0.5) +
  labs(
    x = "Schätzungen für beta_1",
    title = "Verteilung der Schätzungen für beta_1 (2000 Stichproben)"
  )
# + 
...
Here we use the `summarise_all` function to compute the mean for every variable in the simulation dataset, grouping it by term (remember, we have two terms in the output: the coefficients $\hat{\beta_0}$ and $\hat{\beta}$).
...
summary(fit)
```
- 1. Spalte: geschätzte Koeffizienten: $\hat{\beta_0}$, $\hat{\beta}$
...
$$
y_i = \beta_0 + \beta_1 x_i + e_i\\
\beta_1 = 0 \implies y_i = \beta_0 + e_i
$$

Die Aussage $\beta_1 = 0$ bedutet... ?

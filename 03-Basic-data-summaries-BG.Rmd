...
1-ви квартил: около 1/4 от клиентите са били по-ниски от 162.6 см.
3-ви квартил: около 3/4 от клиентите са били по-ниски от 175.9 см.

```{r}
earnings <- earnings %>%
  mutate(
    is_less_than_1qrt = height_cm < 162.6
  ) %>%
  select(height_cm, is_less_than_1qrt, everything())

## Дял на клиенти с ръст по-нисък от 162.6 см.
mean(earnings$is_less_than_1qrt)
```

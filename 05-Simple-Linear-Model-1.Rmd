...
testing_1 <- sim_coeff %>%
  ...
  mutate(
    ## Logical OR: |
    wrong_decision = t_statistic < -3 | t_statistic > 3
  )
## Share of TRUE values
mean(testing_1$wrong_decision)
```
...
sim_coeff %>%
  filter(term == "mom_hs") %>%
    ggplot(aes(x = t_statistic)) + 
      ...
      labs(
        x = "Value of the t-statistic",
        title = "Distribution of t-statistic, wrong null hypothesis (2000 samples)",
        y = ""
      ) +
      geom_density(color = "steelblue") +
      geom_vline(xintercept = 0, color = "red") +
...
testing_2 <- sim_coeff %>%
  ...
  mutate(
    ## Logical AND: &
    wrong_decision = t_statistic < 2 & t_statistic > -2
  )
## Share of TRUE values
mean(testing_2$wrong_decision)
```

...
Difference between two distributions with a low and a high standard deviation.
...
sim_coeff %>%
  ...
  labs(
    x = "Slope estimate",
    title = "Distribution of t-statistic (over 2000 samples)",
    y = ""
  ) +
  geom_density(color = "steelblue") +
...
$$
\text{t-statistic} = \frac{\hat{\beta}_1 - 10}{SE(\hat{\beta}_1)}
$$

$$
t = \frac{11.77 - 10}{2.322} = 0.76
$$

```{r}
sim_coeff <- sim_coeff %>%
  mutate(
    t_statistic = (estimate - 10) / std.error
  )
```
...
The real coefficient equals 11.77 (it is known, because we choose it for the simulation). The hypothesis is that this coefficient is equals to 10, therefore the null hypothesis is not true. The right decision in this case is to reject the null hypothesis.
